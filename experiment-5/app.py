...
def load_311_data(force_refresh=False):
    ...
    df = df[(df['latitude'] > 40) & (df['latitude'] < 43) &
...
def load_shots_fired_data(engine, df_hom, force_refresh=False):
    ...
    matched = confirmed[confirmed["day"].apply(
        lambda d: any((d + pd.Timedelta(days=offset)) in hom_days_set for offset in [-1, 0, 1])
    )]
...
chat_history = [("Assistant", """**City Safety and Service Trends: January 2018**
